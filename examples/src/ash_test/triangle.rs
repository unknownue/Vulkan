...
const API_VERSION:         u32 = vk_make_version!(1, 0, 92);

const WINDOW_TITLE: &'static str = "Ash Test";
...
fn create_surface(entry: &ash::Entry, instance: &ash::Instance, window: &winit::Window) -> SurfaceStuff {
    ...
    use vkbase::platforms::generate_surface;
    let surface = unsafe {
        generate_surface(entry, instance, window).expect("Failed to create surface.")
    };
...
fn setup_debug_callback(is_enable_debug: bool, entry: &ash::Entry, instance: &ash::Instance) -> (ash::extensions::ext::DebugReport, vk::DebugReportCallbackEXT) {
    ...
    if is_enable_debug == false {
        ...
        let debug_call_back = unsafe {
            debug_report_loader.create_debug_report_callback(&debug_create_info, None)
...
fn check_validation_layer_support(entry: &ash::Entry, required_validation_layers: &Vec<&str>) -> bool {
...
fn create_logical_device(instance: &ash::Instance, physical_device: vk::PhysicalDevice, validation: &ValidationInfo, device_extensions: &DeviceExtension, surface_stuff: &SurfaceStuff) -> (ash::Device, QueueFamilyIndices) {
...
fn choose_swapchain_extent(capabilities: &vk::SurfaceCapabilitiesKHR, window: &winit::Window) -> vk::Extent2D {
    ...
    if capabilities.current_extent.width != u32::max_value() {
        ...
        vk::Extent2D {
            width: min(max(window_size.width as u32, capabilities.min_image_extent.width), capabilities.max_image_extent.width),
            height: min(max(window_size.height as u32, capabilities.min_image_extent.height), capabilities.max_image_extent.height)
        }
    }
}
...
fn create_swapchain(instance: &ash::Instance, device: &ash::Device, physical_device: vk::PhysicalDevice, window: &winit::Window, surface_stuff: &SurfaceStuff, queue_family: &QueueFamilyIndices) -> SwapChainStuff {
...
fn is_physical_device_suitable(instance: &ash::Instance, physical_device: vk::PhysicalDevice, surface_stuff: &SurfaceStuff, swapchain_support: &SwapChainSupportDetail, required_device_extensions: &DeviceExtension) -> bool {
...
fn create_image_views(device: &ash::Device, surface_format: vk::Format, images: &Vec<vk::Image>) ->Vec<vk::ImageView> {
    ...
    let swapchain_imageviews: Vec<vk::ImageView> = images.iter().map(|&image| {
        create_image_view(device, image, surface_format, vk::ImageAspectFlags::COLOR, 1)
    }).collect();
...
fn init_window(events_loop: &EventsLoop, title: &str, width: u32, height: u32) -> winit::Window {
